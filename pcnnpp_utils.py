...
def mix_gaussian_loss(x, l):
    """ log-likelihood for mixture of continuous Gaussians, assumes the data has been rescaled to [-1,1] interval
    Args:
        x (Tensor): Target (B x D x D x 3) (B batch size, D dimensions, 1 channel (B/W image)
        y_hat (Tensor): Predictive distribution, (B x D x D x 10*nr_mix) (B batch size, D dimensions, 10 * number of mixture components channels)
