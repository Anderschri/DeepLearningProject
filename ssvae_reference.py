...
parser.add_argument('--calc_nll', type=bool, default=False, help='Calculate NLL?')
...
def loss_components_fn(x, y, z, p_y, p_z, p_x_yz, q_z_xy):
    ...
    #print("1: ", - p_x_yz.log_prob(x).view(x.shape[0], -1).sum(1))
    #print("2: ", - p_y.log_prob(y))
    #print("3: ", - p_z.log_prob(z).sum(1))
    #print("4: ", q_z_xy.log_prob(z).sum(1))
        ...
        for i in range(n_batches):
            ...
            # unlabeled data loss -- SSL paper eq 7
                ...
                for y in q_y.enumerate_support():
                    ...
                    z = q_z_xy.rsample()
                    p_x_yz = model.decode(y, z)
                    L_xy = loss_components_fn(x_flat, y, z, model.p_y, model.p_z, p_x_yz, q_z_xy)
...
def generate(model, dataset, args, epoch=None, n_samples=10):
    ...
    q_z_xy = model.encode_z(x,y, n_samples_per_label) #x.view(n_samples_per_label, -1), y)
    z = q_z_xy.loc
    ...
    if False: #Not working
        ...
        for j in range(n):
            for i in range(n):
                ...
                #y_sample = y_sample.float().cuda()
                z_sample = np.random.normal(size=(1,args.z_dim))
                z_sample = torch.tensor(z_sample).float().cuda() # torch.FloatTensor(z_sample)#.cuda()
                print(y_sample.shape, z_sample.shape)
                x_decoded= model.decode(y_sample, z_sample).probs.view(*args.image_dims)
...
def calc_nll(model, test_data, num_samples):
    ...
    nll = 0
    # num_samples = torch.zeros([num_samples, 1])
    """
    zs = model.p_z.sample(sample_shape=torch.Size([num_samples, args.z_dim]))#.reshape((num_samples, args.z_dim))
    ys = model.p_y.sample(sample_shape=torch.Size([num_samples, 1])).reshape((num_samples, 10))
    decoder = model.decode(ys, zs)
    zs_logprob = torch.mean(model.p_z.log_prob(zs), dim=0)
    ys_logprob = torch.mean(model.p_y.log_prob(ys), dim=0)
    for x in test_data:
        x = torch.tensor(x[0]).cuda().reshape(-1)
        print(decoder.log_prob(x).shape)
        nll -= torch.sum(decoder.log_prob(x))# + torch.sum(zs_logprob) + torch.sum(ys_logprob)
    """
    for x in test_data:
        ...
        for _ in range(num_samples):
            z = q_z_xy.rsample()
            p_x_yz = model.decode(y, z)
            loss += loss_components_fn(x_flat, y, z, model.p_y, model.p_z, p_x_yz, q_z_xy)# *torch.exp(q_z_xy.log_prob(z).sum(1))
